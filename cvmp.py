...
def create_fake_courses(result_dict=None, key=None):
    ...
    expected_courses = len(course_data)
    if existing_courses >= expected_courses:
...
def create_fake_semesters(programs, sessions, result_dict=None, key=None):
    ...
    expected_semesters = sum(p.total_semesters for p in programs) * len(sessions)
    ...
    for program in programs:
        for session in sessions:
            for number in range(1, program.total_semesters + 1):
                if not Semester.objects.filter(program=program, session=session, number=number).exists():
                    try:
                        start_year = session.start_year
                        start_date = date(start_year, 1 if number % 2 == 1 else 7, 1)
                        ...
                        objects_queue.put({
                            ...
                            'end_time': end_date,
                            'is_active': session.is_active and number <= 2
                        })
                    except Exception as e:
                        print(f"Error queuing semester {number} for {program.name} in {session.name}: {e}")
                        continue
    ...
    try:
        ...
        print(f"Error in semester creation threads: {e}")

    try:
        semesters = list(Semester.objects.all())
        print(f"Created {len(semesters) - existing_semesters} new semesters")
        if result_dict is not None and key is not None:
...
def create_fake_student_semester_enrollments(students, semesters, result_dict=None, key=None):
    ...
    expected_enrollments = len(students) * 2
    ...
    for student in students:
        try:
            ...
            max_semesters = min(student.program.total_semesters, max(1, years_elapsed * 2))
            num_semesters = random.randint(1, max_semesters)
...
def create_fake_course_enrollments(semester_enrollments, offerings, result_dict=None, key=None):
    ...
    for enrollment in semester_enrollments:
        try:
            available_offerings = [o for o in offerings if o.semester == enrollment.semester and o.program == enrollment.student.program]
...
def main():
    ...
    # Dependent tasks (can run in parallel after sequential tasks)
    dependent_tasks = [
    ...
    # Run offering-dependent tasks in parallel
    with Pool(processes=4, initializer=worker_init) as pool:
        pool.starmap(execute_task, offering_dependent_tasks)
    ...
    # Sequential tasks: create students and semester enrollments
    ...
    final_tasks = [
        (create_fake_academic_qualifications, (applicants, result_dict, 'qualifications')),
        (create_fake_extra_curricular_activities, (applicants, result_dict, 'extra_activities')),
        (create_fake_assignment_submissions, (assignments, students, result_dict, 'submissions')),
