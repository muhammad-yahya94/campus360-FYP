...
def create_fake_applicants(users, programs):
    applicants = []
    shifts = ['morning', 'evening']
    program_names = [prog for progs in university_programs.values() for prog in progs]
    if not programs:
        print("Warning: No programs available, creation of applicants skipped")
        return applicants
...
def clear_existing_data():
    models_to_clear = [CustomUser, Applicant, Student, Faculty, Department, Program, Semester]
...
def main():
    ...
    applicants = create_fake_applicants(users, programs)
