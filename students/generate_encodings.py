...
def get_embedding_code(session_name, program_name, shift):

    try:
        logger.debug(f"Retrieving/creating EmbeddingsEncode for session: {session_name}, program: {program_name}, shift: {shift}")
        code, created = EmbeddingsEncode.objects.get_or_create(
            session=session_name,
            program=program_name,
            shift=shift
        )
        logger.debug(f"EmbeddingsEncode {'created' if created else 'retrieved'}: {code}")
        return code
    except Exception as e:
        logger.error(f"Error retrieving/creating EmbeddingsEncode: {str(e)}")
        raise
...
def store_embeddings(faces, ecode, cnic):
    """
    ...
    """
    try:
        logger.debug(f"Storing embeddings for cnic: {cnic}")
        existing_embeddings = FaceEmbedding.objects.filter(cnic=cnic)
        if existing_embeddings.exists():
            count = existing_embeddings.count()
            existing_embeddings.delete()
            logger.info(f"Deleted {count} existing embeddings for cnic {cnic}")
        ...
        for i, face in enumerate(faces):
            try:
                ...
                if not embedding or not isinstance(embedding, list) or len(embedding) != 1:
                    logger.error(f"Invalid embedding format for face {i+1} of cnic {cnic}")
                ...
                logger.info(f"Generated embedding {i+1}/{len(faces)} for cnic {cnic}")
            except Exception as e:
                logger.error(f"Error generating embedding for face {i+1} of cnic {cnic}: {e}")
                return False, f"Error generating embedding for face {i+1}: {str(e)}"
        logger.info(f"Successfully processed {len(faces)} faces for cnic {cnic}")
        return True, None
    except Exception as e:
        logger.error(f"Error storing embeddings for cnic {cnic}: {e}")
        return False, f"Error storing embeddings: {str(e)}"

def generate_embeddings(photos, session_name, program_name, shift, cnic):
    try:
        if not photos:
            logger.warning("No photos provided for embedding generation")
            return False, "No photos provided"
        
        # Ensure session_name, program_name, and cnic are valid strings
        session_name = slugify(session_name)
        program_name = slugify(program_name)
        cnic = slugify(cnic)
        logger.debug(f"Sanitized inputs - session: {session_name}, program: {program_name}, cnic: {cnic}")
        # Get or create the embedding code
        logger.debug("Calling get_embedding_code")
        ecode = get_embedding_code(session_name, program_name, shift)
        ...
        if len(faces) < 10:
            logger.warning(f"Not enough faces extracted for cnic {cnic}: {len(faces)} found, at least 10 required")
            return False, f"Not enough faces extracted for cnic {cnic}: {len(faces)} found, at least 10 required"
        # Store embeddings in the database
        logger.debug("Calling store_embeddings")
        success, error = store_embeddings(faces, ecode, cnic)
        if success:
            logger.info(f"Embeddings generated and stored successfully for {len(faces)} faces of cnic {cnic}")
            return True, None
        else:
            return False, error
    except Exception as e:
